...
RUN addgroup -g 1000 node \
    && addgroup -g 2000 nginx \
    && adduser -u 1000 -G node -s /bin/sh -D node \
    && adduser -u 2000 -G nginx -s /bin/sh -D nginx

# Install base packages

RUN apk upgrade \
    && apk add --no-cache git openssl icu-dev zlib-dev libzip-dev sqlite-dev libpng-dev libjpeg-turbo-dev mysql-client libstdc++ \
    && apk add --no-cache --virtual .build-deps \
        autoconf gcc make g++ zlib-dev file g++ libc-dev pkgconf libmemcached-dev binutils-gold curl gnupg libgcc linux-headers python tar
